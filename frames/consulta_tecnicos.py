...
def botao_consultaTec(self):
    ...
    self.bt_show_all = Button(
        self.frame_consultaTec, text='Mostrar Tudo', bg='#F2F2F2', command=lambda: mostrar_tudo(self.listaTec, arquivoCsv))
    self.bt_show_all.place(
        relx=0.06, rely=0.9, relwidth=0.2, relheight=0.05)
    self.bt_limparTec = Button(
        self.frame_consultaTec, text='Limpar', bg='#F2F2F2', command=lambda: limpar(self.listaTec))
    self.bt_limparTec.place(
        relx=0.29, rely=0.9, relwidth=0.2, relheight=0.05)
    self.bt_funcaoFer = Button(
        self.frame_consultaTec, text='Consultar', bg='#F2F2F2', command=lambda: consultar(self.listaTec, self.codigo_entry, arquivoCsv))
    self.bt_funcaoFer.place(
        relx=0.52, rely=0.9, relwidth=0.2, relheight=0.05)
    self.bt_excluirTec = Button(
        self.frame_consultaTec, text='Excluir', bg='#F2F2F2', command=lambda: excluir(self.listaTec, arquivoCsv))
